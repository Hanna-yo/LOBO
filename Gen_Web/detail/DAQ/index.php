...
$nAgentId 		= filter_input_int('aid', 		INPUT_GET);
$sGDetailtoken 	= filter_input_str('Gtoken', 		INPUT_GET);
$sRid 		= filter_input_str('rID',		INPUT_GET);
$sName 		= filter_input_str('acc', 		INPUT_GET);
...
else if(!empty($sGDetailtoken))
{
	...
	$sSQL = "SELECT 	aurl,
		   ...
		   AND	At_Enable = 1 ".sql_limit(0,1);
	$Result = $pdo->prepare($sSQL);
	$Result->bindValue(':At_Code', $nAgentId, PDO::PARAM_INT);
	sql_query($Result);
	$aBack = $Result->fetch(PDO::FETCH_ASSOC);
	if($aBack)
	{
		$sKey = md5(md5($sRoundid.$nAgentId).$aBack['parentid']);
		if($sKey != $sGDetailtoken)
		{
			echo 'Error!';
			$pdo = '';
			exit;
		}
		$sAgent_code = $aBack['agent_code'];
		$sBackUrl = $aBack['aurl'];
	}
	else
	{
...
if($iOk > 1)
{
	...
	#切換資料庫
	$sSQL = "USE `mt_gamerecord_".$sAgent_code."`;";
	$Result = $pdo->prepare($sSQL);
	sql_query($Result);
...
if(isset($_GET['aaa']) && $_GET['aaa'] == 'kkk')
{
	echo json_encode($aRecord);
}
?>
<!doctype html>
<html>
<head>
	...
	<script type="text/javascript" src="../js/cq9_detail<?php echo $sGid;?>.js"></script>
</head>
<body>
